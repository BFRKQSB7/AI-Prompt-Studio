...
- **提示词库参考**：精选库（内嵌 347 条常用标签，默认）/ 全量库（31.7 万条，首次联网加载 `prompt-db.js`，仅浏览器内存检索，不占模型上下文）
- **负面提示词自动生成**：本地与在线模型都实时生成匹配负面（可关闭，关闭时用通用默认），失败回退通用默认
...
- **标签个数上限**：可自定义（默认 20），模型输出超限自动截断（正负面分开算）
- **max_token 可调**：SDXL 与 H3 分开设置（默认 2048）
...
>
> 上下文建议：SDXL 生图 ≥8192，H3 视频 ≥8192（负面提示词也由模型生成）。全量库 31.7 万条仅在浏览器内存检索，不占模型上下文。
...
- **全量库**：31.7 万条（通用 / 画师风格 / 版权作品 / 角色，取自「AI 绘画提示词超市」过滤后全量库）。首次选中联网加载 `prompt-db.js`（约 11.5MB），在浏览器内存检索，每轮只把命中 ≤40 条发送给模型，不占模型上下文。
...
### v1.4.0
- **新增**：负面词生成开关（可关，关闭时用通用默认）
- **新增**：标签个数上限（默认 20，超限自动截断，正负面分开算）
- **新增**：复制 / 清空分列按钮
- **新增**：max_token 可调（SDXL 与 H3 分开设置，默认 2048）
- **新增**：版权角色消歧检索（`CHAR_TAGS`，如 无职转生艾莉丝 → `eris_greyrat`）
- **修复**：sanitizeTags 误删词典词、非全量模式误过滤导致内容丢失

### v1.3.0
- **优化**：全量库更新至 317,423 条（约 11.5MB）
- **版本**：仓库改名 AI-Prompt-Studio

### v1.2.0
- **新增**：标签中文胶囊（正 / 负面，×删除、右键改权重、拖拽排序）
- **新增**：全量库正向后筛选（剔除不在库词条）+ 禁自造变体
- **修复**：全量库角色名检索——先精确匹配通用词再前缀展开角色变体（阿米娅/博士等正确命中）

### v1.1.0
- **新增**：提示词库参考（精选 / 全量库）模块
- **新增**：H3 视频提示词模块（T2VA / I2VA / FL2VA / L2VA / Ref2VA）
- **优化**：SDXL 正向标签过全量库筛选

### v1.0.0
- **版本**：首个正式发布（版本徽章 + 界面版本号）
